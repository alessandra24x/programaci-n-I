...
static int addNode(LinkedList* this, int nodeIndex,void* pElement) {
    ...
    if(this != NULL && nodeIndex >= 0 && nodeIndex <= ll_len(this)) {
    	...
    	if(nodeIndex == 0) { // Si es la primera posición se agregará el nodo al principio de la lista
    		pAuxNode = getNode(this, nodeIndex); // Obtengo el nodo de la posición indicada por parametro
    		pNewNode->pElement = pElement; // Al nuevo nodo en su campo pElement le guardo un puntero al elemento recibido por parametro
    		pNewNode->pNextNode = pAuxNode; // Al campo NextNode de Node le asigno el apuntador al nodo obtenido con getNode
    		this->pFirstNode = pNewNode; // Se asigna a la cabecera de la lista (puntero al primer nodo) el nodo creado anteriormente
    		this->size++; // Al campo Size se le aumenta en 1 su longitud para saber la longitud exacta de la lista
    	} else { // Si no es la primera posición el procedimiento es distinto
    		pAuxNode = getNode(this, nodeIndex -1); // Obtengo el nodo anterior a la posición indicada por parametro (nodeIndex - 1)
    		pNewNode->pElement = pElement; // Al nuevo nodo en su campo pElement le guardo un puntero al elemento recibido por parametro
    		pNewNode->pNextNode = pAuxNode->pNextNode; // Al campo NextNode del nuevo nodo le asigno el apuntador al nodo siguiente que esta siendo apuntando desde el nodo auxiliar
    		pAuxNode->pNextNode = pNewNode; // Al campo NextNode del nodo auxiliar le asigno el apuntador al nuevo nodo
    		this->size++; // Al campo Size se le aumenta en 1 su longitud para saber la longitud exacta de la lista
    	}
    	returnAux = 0;
    }
    ...
    if(this != NULL && pFunc != NULL && ll_len(this) > 1 && (order == 1 || order == 0)) {
    	do {
    		...
    		for(i = 0; i < ll_len(this)-1; i++) {
                ...
                comparisson = pFunc(buffer1->pElement, buffer2->pElement);

    			if(order == 0 && comparisson < 0 || order == 1 && comparisson > 0) {
...
LinkedList* ll_map(LinkedList* this,void* (*pFunc)(void* pElement)) {
	...
	if(this != NULL && pFunc != NULL) {
        returnAux = ll_newLinkedList();
		for(i = 0; i < ll_len(this); i++) {
            pAuxElement = pFunc(ll_get(this,i));
			if(pAuxElement != NULL) {
                ll_add(returnAux, pAuxElement);
            }
		}
